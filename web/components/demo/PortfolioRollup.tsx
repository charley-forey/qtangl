      ...
      <div className="flex items-end justify-between gap-3">
        ...
        {overallReadiness != null ? (
          <p className="text-xs text-[var(--color-gray-400)]">
            Full report readiness: {overallReadiness}/100 · {overallBand}
          </p>
        ) : null}
      </div>
        ...
        {units.map((unit) => (
          <div key={unit.businessUnit}>
            <div className="flex justify-between text-xs text-[var(--color-gray-400)]">
              ...
              <span>{unit.readinessScore ?? "Readiness unavailable"} · {unit.assetCount} assets</span>
            </div>
            {unit.readinessScore != null ? (
              <div className="mt-1 h-2 rounded-full bg-white/10">
                <div
                  className="h-2 rounded-full bg-cyan-400/80"
                  style={{ width: `${Math.max(0, Math.min(100, unit.readinessScore))}%` }}
                />
              </div>
            ) : null}
          </div>
