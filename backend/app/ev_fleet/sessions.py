...
def _ttl_seconds() -> int:
    raw = os.getenv("QTANGL_EV_FLEET_SESSION_TTL_SECONDS") or os.getenv("QTANGL_HOSPITAL_SESSION_TTL_SECONDS")
    if not raw:
...
def _get_store():
    global _store
    if _store is None:
        _store = session_store(namespace="ev_fleet", ttl_seconds=_ttl_seconds(), prefix="ev-fleet")
    return _store
