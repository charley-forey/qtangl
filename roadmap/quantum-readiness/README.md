...
```mermaid
flowchart LR
  Assess["Assess: scan + CBOM + Mosca + readiness score"] --> Monitor["Monitor: scheduled re-scans + drift diff + alerts"]
  Monitor --> Convert["Convert: prioritized remediation + guided migration + re-scan proof"]
  Convert --> Evidence["Evidence: signed reports + verify link + audit packs"]
  Evidence --> Monitor
  Convert -.-> Optimize["Later: hybrid optimization expansion"]
```
